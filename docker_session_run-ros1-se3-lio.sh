...
RECORDED_BAG_NAME="recorded-se3-lio.bag"
...
source /ros_ws/devel/setup.bash
echo "[record] start"
rosbag record /local/cloud_registered_body /local/odometry -O '"$BAG_OUTPUT_CONTAINER/$RECORDED_BAG_NAME"'
echo "[record] exit"
'\'' C-m
...
docker run -it --rm \
  ...
  /bin/bash -c "
    ...
    rosrun se3-lio-to-hdmapping listener \
      ...
      \"$BAG_OUTPUT_CONTAINER/$HDMAPPING_OUT_NAME-SE3-LIO\"
  "
